...
def write_scatter(fname: str, jsons: dict,
  ...
  for dirname, data_list in jsons.items():
    for data in data_list:
      ...
      if total_gift == 0:
        continue
      xy.append([total_gift, livescore])

  # x でソートする
  xy = sorted(xy, key=lambda v: v[0])

  xinvalid = set()
  for i, (total_gift, livescore) in enumerate(xy):
  ...
  if plot_rate:
    ...
    ax2.scatter(xx, y_livescore_per_gift, label='Real score / gift',
                color='#FFD700', alpha=0.5, s=3)
